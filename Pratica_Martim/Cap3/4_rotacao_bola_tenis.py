...
r0 = np.array([-10.0, 1.0, 0.0]) # vetor posição inicial [m]
v0 = 130.0 * 1000 / 3600         # velocidade inicial [m/s]
theta = np.deg2rad(10)           # ângulo com a horizontal
m = 0.057                        # massa da bola de ténis [kg]
R = 0.067/2                      # raio da bola de ténis [m]
...
w0 = np.array([0.0, 0.0, 0.0])    # a) Sem rotação       -- vetor w
w1 = np.array([0.0, 0.0, 100.0])  # b) Rotação positiva  -- vetor w
w2 = np.array([0.0, 0.0, -100.0]) # c) Rotação negativa  -- vetor w
