'''
Maria Inês Gonçalves
124754
'''

import numpy as np
import matplotlib.pyplot as plt

'''
Gráfico do logaritmo da massa em função do comprimento
'''
C = np.array([1.2, 4.2, 11, 20, 22, 37, 45]) #eixo x  --> comprimento do fémur (cm)
M = np.array([0.03, 0.54, 9.1, 38, 57, 230, 480]) #eixo y --> massa (kg)

x = C
y = M

npontos = len(C)

y = np.log(y) # transformar em log(y) grafico SEMI-LOG

xy = x*y
xx = x**2
yy = y**2

sx = x.sum()
sy = y.sum()
sxy = xy.sum()
sxx = xx.sum()
syy = yy.sum()

n = npontos
m = (n*sxy - sx*sy)/(n*sxx - sx**2)
b = (sxx*sy - sx*sxy)/(n*sxx - sx**2)
r2 = (n*sxy - sx*sy)**2/((n*sxx - sx**2)*(n*syy - sy**2))

errom= np.abs(m)*np.sqrt(((1/r2)-1)/(n-2)) #erro associado ao declive
errob = errom*np.sqrt(sxx/n) #erro associado à ordenada na origem

print("GRÁFICO DO LOGARITMO DA MASSA EM FUNÇÃO DO COMPRIMENTO")
print("declive m = ", m, "+/-", errom)
print("erro associado ao declive = ",errom)
print("coeficiente de determinação r**2 = ", r2)

# mostrar o gráfico com os pontos e a reta da regressão linear
equacao=m*x+b
plt.plot(x, equacao,label="Reta de regressao linear") # reta
plt.scatter(x, y, marker='o', color='m',label="Pontos") # pontos
plt.xlabel("Comprimento do fémur (cm)")
plt.ylabel("")
plt.legend()
plt.show()

print("---------------------------------------------")
'''
Gráfico do logaritmo da massa em função do logaritmo do comprimento
'''
x = np.log(x) # transformar em log(x) e como lá em "cima" o y já está transformado
              # em log, fica para o gráfico log-log
xy = x*y
xx = x**2
yy = y**2

sx = x.sum()
sy = y.sum()
sxy = xy.sum()
sxx = xx.sum()
syy = yy.sum()

n = npontos
m = (n*sxy - sx*sy)/(n*sxx - sx**2)
b = (sxx*sy - sx*sxy)/(n*sxx - sx**2)
r2 = (n*sxy - sx*sy)**2/((n*sxx - sx**2)*(n*syy - sy**2))

errom= np.abs(m)*np.sqrt(((1/r2)-1)/(n-2)) #erro associado ao declive
errob = errom*np.sqrt(sxx/n) #erro associado à ordenada na origem

print("GRÁFICO DO LOGARITMO DA MASSA EM FUNÇÃO DO LOGARITMO DO COMPRIMENTO")
print("declive m = ", m, "+/-", errom)
print("erro associado ao declive = ",errom)
print("coeficiente de determinação r**2 = ", r2)

# mostrar o gráfico com os pontos e a reta da regressão linear
equacao=m*x+b
plt.plot(x, equacao,label="Reta de regressao linear") # reta
plt.scatter(x, y, marker='o', color='m',label="Pontos") # pontos
plt.xlabel("Comprimento do fémur (cm)")
plt.ylabel("Massa (kg)")
plt.legend()
plt.show()