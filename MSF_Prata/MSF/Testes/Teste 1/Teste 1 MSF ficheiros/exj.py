...
while y[i] >= 0:
    ...
    axr[i+1] =ax[i] - (g/vterminal2 * math.sqrt(vxr[i]2+vyr[i]2) *vxr[i]))
    ayr[i+1] =ay[i] - (g/vterminal2 * math.sqrt(vxr[i]2+vyr[i]2) vyr[i]))

    xr[i+1]=xr[i]+vxr[i]*deltat+(1/2*axr[i]*deltat2)
    yr[i+1]=yr[i]+vyr[i]*deltat+(1/2*ayr[i]*deltat2)
